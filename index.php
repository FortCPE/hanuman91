                ...
                if($fetch_connection['status'] == 'true'){
                    if($groupId != '' && $userId != ''){
                        if($text == 'Bot Shutdown'){
                            ...
                            $query_count = $pdo->prepare("SELECT * FROM bot_customer WHERE user_id = :user_id AND `today` = :today");
                            $query_count->execute(Array(
                                ":user_id" => $userId,
                                ":today" => date("Y-m-d")
                            ));
                            $count_row = $query_count->rowCount();
                            if($count_row >= 1){
                                $delete_connection = $pdo->prepare("DELETE FROM `bot_customer` WHERE `user_id` = :user_id AND `today` = :today");
                                $result = $delete_connection->execute(Array(
                                    ":user_id" => $userId,
                                    ":today" => date("Y-m-d")
                                ));
                                if($result){
